...
void AVLTree<K, V>::rebalance(Node*& subtree)
{
    ...
    int b = (heightOrNeg1(subtree->left)-heightOrNeg1(subtree->right));//+1
    if(b == -2)
    {
      int r = (heightOrNeg1(subtree->right->left)-heightOrNeg1(subtree->right->right));//+1
      if (r == -1) {
        rotateLeft(subtree);
      } else {
    ...
    if(b == 2)
    {
      int L = (heightOrNeg1(subtree->left->left)-heightOrNeg1(subtree->left->right));//+1
      if (L == 1) {
        rotateRight(subtree);
      } else {
